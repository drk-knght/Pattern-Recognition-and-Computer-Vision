...
public:
    ...
    void adjustParameters(char key) {
        switch(key) {
            ...
            // Harris parameters
            case 'q': threshold = std::max(0.0, threshold - 10); break;
            case 'w': threshold += 10; break;
            case 'a': blockSize = std::max(2, blockSize - 1); break;
            case 's': blockSize++; break;
            
            // FAST parameters
            case 'z': fastThreshold = std::max(1, fastThreshold - 1); break;
            case 'x': fastThreshold++; break;
            case 'n': nonmaxSuppression = !nonmaxSuppression; break;
        }
    }
};
