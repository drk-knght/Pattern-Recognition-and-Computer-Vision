/*
    ...
    This file implements a comprehensive image recognition system that extracts features from target images and classifies objects using a K-Nearest Neighbor (KNN) classifier with K > 1. It includes robust functionality for managing and updating a training database, preprocessing images through techniques like thresholding and erosion, and visualizing classification results. The system supports both training and classification modes, allowing for dynamic interaction with image data and providing detailed feedback on object recognition performance.
*/
...
// Extra Credit: Connected Components Analysis implemented from scratch.
// This function assumes an 8-connected neighborhood and a binary image (CV_8UC1) with
// foreground = 255 and background = 0.
cv::Mat connectedComponentsCustom(const cv::Mat &binary)
{
    CV_Assert(binary.type() == CV_8UC1);
    cv::Mat labels = cv::Mat::zeros(binary.size(), CV_32SC1);
    int nextLabel = 1;
    // Using a vector for the union-find structure.
    // The maximum possible number of labels is theoretically the number of foreground pixels,
    // but here we start with a reasonable size.
    std::vector<int> labelEquiv(binary.rows * binary.cols / 2 + 1, 0);

    // Helper lambda functions for union-find.
    auto find = [&labelEquiv](int x) -> int
    {
        while (labelEquiv[x] != x)
        {
            x = labelEquiv[x];
        }
        return x;
    ...
    auto unionLabels = [&labelEquiv, &find](int a, int b)
    {
        int rootA = find(a);
        int rootB = find(b);
        if (rootA < rootB)
            labelEquiv[rootB] = rootA;
        else if (rootA > rootB)
            labelEquiv[rootA] = rootB;
    };

    // First pass: assign preliminary labels and record label equivalences.
    for (int i = 0; i < binary.rows; ++i)
    {
        for (int j = 0; j < binary.cols; ++j)
        {
            if (binary.at<uchar>(i, j) == 255)
            {
                ...
                // Check the 8-connected neighbors (only previously visited ones):
                // Top-left, top, top-right, and left.
                for (int di = -1; di <= 0; ++di)
                {
                    for (int dj = -1; dj <= (di == 0 ? -1 : 1); ++dj)
                    {
                        int ni = i + di, nj = j + dj;
                        if (ni >= 0 && ni < binary.rows && nj >= 0 && nj < binary.cols)
                        {
                            if (binary.at<uchar>(ni, nj) == 255)
                            {
                ...
                {
                    // No labeled neighbor: assign a new label.
                    labels.at<int>(i, j) = nextLabel;
                    labelEquiv[nextLabel] = nextLabel;
                    ++nextLabel;
                }
                else
                {
                    ...
                    for (int lab : neighborLabels)
                    {
                        if (lab != minLabel)
                        {
                            unionLabels(minLabel, lab);
                        }
                    }
                }
            }
    ...
    for (int i = 0; i < binary.rows; ++i)
    {
        for (int j = 0; j < binary.cols; ++j)
        {
            ...
            {
                labels.at<int>(i, j) = find(labels.at<int>(i, j));
            }
        }
    }

    // Optional: Relabel components to have sequential labels (1,2,3,...).
    ...
    for (int i = 0; i < labels.rows; ++i)
    {
        ...
        {
            int lab = labels.at<int>(i, j);
            if (lab != 0)
            {
                if (labelMapping.find(lab) == labelMapping.end())
                {
                    labelMapping[lab] = labelIdx++;
                }
                labels.at<int>(i, j) = labelMapping[lab];
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    for (const auto &entry : fs::directory_iterator(imageDir))
    {
        ...
        {
            for (int j = 0; j < labelsCustom.cols; ++j)
            {
                int label = labelsCustom.at<int>(i, j);
                if (label > 0)
                {
                    // Create color mapping if it doesn't exist.
                    if (labelColors.find(label) == labelColors.end())
                        labelColors[label] = cv::Scalar(rand() % 256, rand() % 256, rand() % 256);
                    dstCustom.at<cv::Vec3b>(i, j) = cv::Vec3b(labelColors[label][0], labelColors[label][1], labelColors[label][2]);
                }
            }
        }
        cv::imshow("Custom Segmentation", dstCustom);
        cv::waitKey(0);
    }
